...
func update():
	...
	(beam_material as ShaderMaterial).set_shader_parameter("texture_scale", beam_length)
	(beam_material as ShaderMaterial).set_shader_parameter("pulse_offset", randf() * 10)
	(beam_material as ShaderMaterial).set_shader_parameter("pulse_speed", 3 + randf() * 5)
